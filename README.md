...
- [Возможности](#возможности)
  - [DSL для построения деревьев](#dsl-для-построения-деревьев)
...
- встроенный DSL на макросах для компактной записи правил дифференцирования и преобразований дерева.

### DSL для построения деревьев

Для работы с деревьями в проекте используется небольшой внутренний DSL поверх C-макросов. Конструкции `ADD_`, `MUL_`, `POW_`, `SIN_`, `NUM_` и другие создают узлы соответствующих типов, а `CL`/`CR` и `DL`/`DR` обозначают соответственно копии и производные левого и правого поддеревьев.

Например, правило произведения записывается одной строкой:

```cpp
return ADD_(MUL_(DL, CR), MUL_(CL, DR));
```

Эта запись соответствует формуле `f'g + fg'`.

Такой DSL делает правила наглядными, уменьшает количество шаблонного кода и упрощает добавление новых преобразований дерева.
...
Парсер преобразует список токенов в бинарное дерево с помощью алгоритма рекурсивного спуска. Каждому нетерминалу грамматики соответствует отдельная функция парсера:
...
Здесь `N` - число, `V` - имя переменной из латинских букв, а `$` - внутренний маркер конца входной строки, который пользователь не вводит. Пробельные символы между токенами игнорируются.

Уровни грамматики задают приоритет операций: функции и скобки обрабатываются раньше возведения в степень, затем идут умножение и деление, после них - сложение и вычитание. Бинарные операции одного уровня, включая `^`, группируются слева направо.
...
<details open>
  ...
  <br>
  <a href="assets/derivative_tree_example.svg">
    <img src="assets/derivative_tree_example.svg" alt="Дамп дерева производной" width="100%">
  </a>
</details>
