...
static const char* token_type_to_string(type_data type);
...
void list_dump(list_t* const list, const variable_t* const variables)
{
    ...
    for (const token_t* token = list->head; token; token = token->next)
    {
        ...
        fprintf(txt_file, "node_%zu [shape=plain, group=\"column_%zu\", label=<<TABLE "
                          ...
                          TOKEN_NODE_WIDTH, token_type_to_string(token->type));
        ...
        switch (token->type)
        {
            ...
            case VAR:
                fprintf(txt_file, "VAR_NUM = %d (", token->data_t.var_number);
                print_html_escaped(txt_file, variables[token->data_t.var_number].name);
                fprintf(txt_file, ")");
                break;
            ...
            case SPEC:
                fprintf(txt_file, "VALUE = %c", token->data_t.spec_symbol);
                break;
        ...
        fprintf(txt_file, "<TR><TD COLSPAN=\"2\" WIDTH=\"%zu\">ADDRESS = %p</TD></TR>"
    ...
    char command[1000] = {};
    sprintf(command, "dot \"" LIST_DUMP_TXT "\" -Tsvg -o \"" LIST_DUMP_SVG "\"");
    system(command);
